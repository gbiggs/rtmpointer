...
bool RTC::addPointerPort(char const* name, PointerPortBase& port,
        RTObject_impl& rtc)
{
    std::string propkey("port.pointerport.");
    propkey += name;
    coil::Properties& props(rtc.getProperties());
    props.getNode(propkey) << props.getNode("port.pointerport");

    bool result = rtc.addPort(port);
    if (!result)
    {
        std::cerr << "addPointerPort() failed\n";
        return result;
    }
